...
export function calculateUnifiedTax(input: TaxAssessmentInput): TaxAssessmentResult {
  ...
  /**
   * FIX: Define the explicit "width" (chunk size) of each tax band.
   * This completely avoids the progressive cumulative calculation errors.
   */
  const brackets = [
    { width: 800000, rate: 0.0, label: 'Exempt (First ₦800k)' },
    { width: 2200000, rate: 0.15, label: '₦800k to ₦3M @ 15%' },     // 3,000,000 - 800,000
    { width: 9000000, rate: 0.18, label: '₦3M to ₦12M @ 18%' },     // 12,000,000 - 3,000,000
    { width: 13000000, rate: 0.21, label: '₦12M to ₦25M @ 21%' },   // 25,000,000 - 12,000,000
    { width: 25000000, rate: 0.23, label: '₦25M to ₦50M @ 23%' },   // 50,000,000 - 25,000,000
    { width: Infinity, rate: 0.25, label: 'Above ₦50M @ 25%' }
  ];
  ...
  // Loop systematically processes exact bracket allocations
  for (const bracket of brackets) {
    if (remainingIncome <= 0) break;

    const taxableAmount = Math.min(remainingIncome, bracket.width);
    const taxForBracket = taxableAmount * bracket.rate;
  ...
  // If no taxable items were pushed (e.g. income is 0), populate a fallback row
  if (breakdown.length === 0) {
    breakdown.push({ bracket: 'Exempt (Taxable income is ₦0)', rate: 0.0, taxableAmount: 0, tax: 0 });
  }

  const isExempt = taxableIncome <= 800000;
  const netIncome = annualGross - computedTax - pensionDeduction;

  // ─── Comparative Old Law Analytics (Cleaned Up) ───────────────────────────
  const oldCRA = Math.max(200000, annualGross * 0.01) + (annualGross * 0.20);
  const oldTaxableIncome = Math.max(0, annualGross - oldCRA - pensionDeduction);
  
  const oldBands = [
    { width: 300000, rate: 0.07 },
    { width: 300000, rate: 0.11 },
    { width: 500000, rate: 0.15 },
    { width: 500000, rate: 0.19 },
    { width: 1600000, rate: 0.21 },
  ...
  for (const band of oldBands) {
    if (oldRemaining <= 0) break;
    const amt = Math.min(oldRemaining, band.width);
    oldTax += amt * band.rate;
    oldRemaining -= amt;
  }
